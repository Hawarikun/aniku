...
class TopAnimeResponse {
  ...
  factory TopAnimeResponse.fromJson(Map<String, dynamic> json) =>
      TopAnimeResponse(
        ...
        data: List<Now>.from(json["data"].map((x) => Now.fromJson(x))),
      );
...
class Now {
  ...
  factory Now.fromRawJson(String str) => Now.fromJson(json.decode(str));
...
enum Rating { PG_13_TEENS_13_OR_OLDER, R_17_VIOLENCE_PROFANITY }
...
final seasonValues = EnumValues({"fall": Season.FALL});

enum Source { LIGHT_NOVEL, MANGA }

final sourceValues =
    EnumValues({"Light novel": Source.LIGHT_NOVEL, "Manga": Source.MANGA});
...
enum TitleType { DEFAULT, ENGLISH, JAPANESE, SYNONYM }
...
enum NowType { MOVIE, ONA, TV }

final NowTypeValues = EnumValues(
    {"Movie": NowType.MOVIE, "ONA": NowType.ONA, "TV": NowType.TV});
